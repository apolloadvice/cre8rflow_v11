...
class CutOperationHandler(BaseOperationHandler):
    ...
    def execute(self, operation: EditOperation, executor) -> ExecutionResult:
        ...
        if (not clip_name and not clip_id) or timestamp_frames is None:
            return ExecutionResult(False, "Missing clip name/id or timestamp for CUT operation.")
        ...
        return ExecutionResult(result, f"Cut operation on {clip_name or clip_id} at {timestamp_frames} frames: {result}") 