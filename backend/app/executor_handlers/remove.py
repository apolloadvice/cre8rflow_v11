...
class RemoveOperationHandler(BaseOperationHandler):
    ...
    def execute(self, operation: EditOperation, executor) -> ExecutionResult:
        ...
        result = executor.timeline.remove_clip(clip_name, track_type=track_type, track_index=track_index)
        if result:
            return ExecutionResult(True, f"Removed clip '{clip_name}' from {track_type} track.")
        else:
            return ExecutionResult(False, f"Clip '{clip_name}' not found in {track_type} track.") 